...
// Persists T-Line drawings per symbol AND panel (modeTag), so panel 1 (1h) and panel 2 (15m RTH)
// each keep their own fully independent set of lines instead of sharing one file — multiple lines
// per panel are allowed (no "1 at a time" cap). Times are the same "ET digits disguised as UTC"
// epoch seconds chart.html already works in (see ChartPanel.ToChartJson) — stored and round-tripped
...
internal static class TLineStore
{
    ...
    private static string PathFor(string symbol, string modeTag) => Path.Combine(OutputFolder, symbol, $"{symbol}_TLines_{modeTag}.csv");

    public static List<(long T1, decimal P1, long T2, decimal P2)> Load(string symbol, string modeTag)
    {
        var path = PathFor(symbol, modeTag);
    ...
    public static void Append(string symbol, string modeTag, long t1, decimal p1, long t2, decimal p2)
    {
        var path = PathFor(symbol, modeTag);
    ...
    public static void Clear(string symbol, string modeTag)
    {
        var path = PathFor(symbol, modeTag);
    ...
    public static void Remove(string symbol, string modeTag, long t1, decimal p1, long t2, decimal p2)
    {
        var path = PathFor(symbol, modeTag);
        var existing = Load(symbol, modeTag);
